...
% net3 = layrecnet(1:2,[ nna nnb nnc]);
net3 = layrecnet(1:2,[ 20 15 5]);
net3.divideParam.valRatio = 5/100;
net3.trainParam.epochs=1000;
net3.trainParam.max_fail=30;
...
%%%% result train data
ytrainYhatrainpredicted1=output2*max;
Yntrain=Ytrain*max;
...
% 
%         dirname=['ResultRNN/Data' num2str(i) '/Workspace'];
%         mkdir(dirname);
% 
%         filename=strcat(dirname,'/set_',num2str(F),'.mat');
%         save(filename);
% 
%         dirname=['ResultRNN/Data' num2str(i) '/Figures'];
%         mkdir(dirname);
% 
